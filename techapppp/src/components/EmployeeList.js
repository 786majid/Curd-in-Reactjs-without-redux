...
const EmployeeList = () => {
    ...
    const [item, setItem] = useState({})                   //it is for id.because jb hm id dete hai to ye id ksi object me store hota hai.       //we make a new object (item k naam se)jse hi me save krunga to item ke andr mera id leke jyega.means hr time(save) pe ek new id leke mera pura value(information) iske andr(item) chla jyega.ab mera final object item keyword ban gaya.ab is object(item) ko array ke andr push karna hoga.it means item ko browser me dikhane k liye le jaa rhe h 

    //for insert/create:-
    const insertSave = () => {                                       //insertSave keyword  for insert the data/value

        // for set/save  the input inside this we simply pass the
        setData([...data, {
            id: Math.floor((Math.random() * 100) + 1),                //aise krne se ek id create ho jaata hai.or ye ek object me store hota hai.
            ...
            phone: value.phone
        }]);                                   // data naam ke object me gye or item naam k object ko pass kr diye
        handleClose()                           //direct data  me value save kr diye (that is yhn mne data save kiya)
    };


    // for put up the data for edit:-
    ...
    const handleUpdate = () => {
        var info = data.map((option) => {
            if (value.id == option.id) {                    //value ka id agr array ke id(option.id)se equal ho jaye to hm wo previous data(information) ko is new wale data(information) se replace kr de rhe h
                return {                                       // for replace old value to new value
                    ...option,                                   //for new information
                    id: value.id,
                    name: value.name,
                    email: value.email,
                    phone: value.phone,
            ...
            return option;                      // after replace, we simply return the new information                                                             
        })
        // save  the input after update(edit).
        setData(info);                                                                                                      //it is my main array(item)
        handleClose()
    }
                    ...
                    <Modal.Body>
                        <Form.Group>
                            <Form.Control
                                type='hidden'
                                placeholder='Name *'
                                value={value.id}        //it is called put the value into form
                                onChange={(e) => {
                                    console.log(e)
                            ...
                            <Form.Control
                                ...
                                placeholder='Name *'
                                // required
                                value={value.name}     //it is called put the value into form
                                onChange={(e) => {
                        ...
                        <Form.Group>
                            <Form.Control
                                ...
                                value={value.email}
                                onChange={(e) => {
                                    setValue({ ...value, email: e.target.value })                                                                            //previous data ko rkh k or new data daal rahe hai.
                                }}
                            />
                        </Form.Group>
                        <Form.Group>
